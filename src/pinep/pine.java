...
public class pine extends PollingScript<ClientContext> implements MessageListener {
    ...
    public void poll() {
        if(ctx.inventory.select().id(2114).count()==0){
            ...
            if (ctx.bank.opened() && ctx.bank.id(2114).select().count()<6) {
        ...
        if (!stop && ctx.players.local().animation()==-1) {
            ...
            if(c.visible()) {
                ...
                Condition.sleep(Random.nextInt(100, 150));
                    ...
                    Condition.sleep(Random.nextInt(100, 150));
                    Item pine = ctx.inventory.select().id(2114).poll();
                    pine.hover();
                    pine.click();
